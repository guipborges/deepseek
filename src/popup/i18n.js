// i18n for popup — loaded via <script> before popup.js
...
function isSupportedTranslationLanguage(value) {
  ...
  return TRANSLATION_LANGUAGES.some((l) => l.value.toLowerCase() === n);
}
...
function populateLanguageSelect(select, { includeAuto = false, useShortLabels = false } = {}) {
  ...
  if (includeAuto) {
    ...
    opt.value = "auto"; opt.textContent = "Auto"; select.appendChild(opt);
  }
  for (const lang of TRANSLATION_LANGUAGES) {
    const opt = document.createElement("option");
    opt.value = lang.value;
    opt.textContent = useShortLabels ? lang.short : lang.label;
    select.appendChild(opt);
  }
  if (current && (current === "auto" || isSupportedTranslationLanguage(current))) select.value = current;
}
...
function populateTranslationLanguageSelects() {
  ...
  if (!quickTargetLanguageSelect.value) quickTargetLanguageSelect.value = "pt-BR";
}

function getSourceLanguageShortLabel(language) {
  const code = (language || "auto").toLowerCase();
  if (code === "auto") return "AUTO";
  const match = TRANSLATION_LANGUAGES.find((l) => l.value.toLowerCase() === code);
  if (match?.short) return match.short;
  return code.slice(0, 2).toUpperCase() || "AUTO";
}

function updateSaveFlashcardButtonLabel() {
  const short = getSourceLanguageShortLabel(quickSourceLanguageSelect.value || "auto");
  saveWordBtn.textContent = t("saveFlashcard").replace("{lang}", short);
}
...
function applyPopupLanguage() {
  ...
  if (voicePickerBtn) { voicePickerBtn.title = t("voicePickerTitle"); voicePickerBtn.setAttribute("aria-label", t("voicePickerTitle")); }
  if (voiceSettingsBtn) { voiceSettingsBtn.title = t("voiceSettingsTitle"); voiceSettingsBtn.setAttribute("aria-label", t("voiceSettingsAria")); }
  ...
  if (tabsNav) tabsNav.setAttribute("aria-label", t("tabsAria"));
  ...
  if (forvoBtn) forvoBtn.textContent = t("forvo");
  generateDetailsBtn.textContent = t("generateDetails");

  const dt = document.querySelector(".details-title");
  if (dt) dt.textContent = t("detailsTitle");

  const dls = document.querySelectorAll(".detail-label");
  if (dls.length >= 7) {
    dls[0].textContent = t("pronunciationSimple");
    dls[1].textContent = t("synonym");
    dls[2].textContent = t("antonym");
    dls[3].textContent = t("example1");
    dls[4].textContent = t("example2");
    dls[5].textContent = t("pastExample");
    dls[6].textContent = t("futureExample");
  }

  for (const btn of [detailExample1AudioBtn, detailExample2AudioBtn, detailPastAudioBtn, detailFutureAudioBtn]) {
    if (!btn) continue;
    btn.title = t("detailExampleAudioTitle");
    btn.setAttribute("aria-label", t("detailExampleAudioTitle"));
  }

  const tt = tabTranslatorBtn.querySelector("span:last-child");
  const dt2 = tabDetailsBtn.querySelector("span:last-child");
  if (tt) tt.textContent = t("tabTranslator");
  if (dt2) dt2.textContent = t("tabDetails");

  if (historySelect.options.length) historySelect.options[0].textContent = t("historyPlaceholder");
  renderHistorySelect();
  if (voiceSelect?.options?.length) { voiceSelect.options[0].textContent = t("autoVoiceOption"); updateVoicePickerState(); }
...
function applyOnboardingLanguage() {
  ...
  if (onboardingSteps) {
    ...
    for (const step of Array.isArray(steps) ? steps : []) {
      ...
      li.textContent = step;
      onboardingSteps.appendChild(li);
    }
  }
}
...
function applyTheme(themeMode) {
  const n = (themeMode || "light").toLowerCase();
  document.body.setAttribute("data-theme", n === "dark" ? "dark" : "light");
}
